...
![picture 1](Media/b73f307aae3d98982398b7190cf77264eb298ffb295197ceae9baf5258935d0c.png)  
...
2. SSAO是一种基于屏幕空间的方法, 也就是其是后处理的, 计算复杂度与场景中三角面片的数量无关
3. SSAO的思路和真正的AO不同, 其非常trick, 本质上更接近于边缘检测算法
4. 这里实现的方法是基于得到的深度图, 朝周围几个方向进行搜索, 取得当前像素周围一定方向内最大的深度差, 计算出对应的角度就认为是这个像素的光线遮蔽角度
5. 遮蔽角度大的像素代表越不可能被环境光照到, 所以就对那个像素进行暗化处理
6. 经过这样的处理SSAO能起到让场景深度大幅变化的部位变暗, 起到模拟环境光的效果
7. 实践中常用的SSAO并不只是这么简单, 其是根据目标像素周围的固定一些点进行采样以便加速, 且采样点与法线方向有关减少自遮蔽走样, 采样得到的环境光权值图还需要经过平滑滤波进一步处理, 后话